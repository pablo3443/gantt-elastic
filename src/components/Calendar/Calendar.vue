...
export default {
  ...
  methods: {
    ...
    generateWeeks() {
      ...
      let currentDate = dayjs(this.root.state.options.times.firstTime); // 19 мая(старт) среда
      for (let weekIndex = 0; weekIndex < weeksCount.count; weekIndex++) { // для каждой недели
        let weekWidth = 0;
        let weekOffset = Number.MAX_SAFE_INTEGER;
        let finalDate = dayjs(currentDate) // начальный день каждой недели
          .add(1, 'week')
          .startOf('week');
        ...
        if (currentDate.valueOf() > this.root.state.options.times.lastTime) {
          currentDate = dayjs(this.root.state.options.times.lastTime); // окончание графика
        }
      }
