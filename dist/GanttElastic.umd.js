...
/* harmony default export */ var Calendarvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    howManyWeeksFit() {
        ...
        if (weeksCount === 1) { // если неделя одна
          for (let formatName of formatNames) {
    ...
    generateWeeks() {
      ...
      const weeksCount = this.howManyWeeksFit(); // количество и тип недель
      if (weeksCount.count === 0) {
        return weeks;
      }
      const steps = this.root.state.options.times.steps; // массив клеток на графике
      const localeName = this.root.state.options.locale.name; // 'ru'
      let formatNames = Object.keys(this.root.state.options.calendar.week.format); // ['long', 'medium', 'short']
      let currentDate = dayjs_min_default()(this.root.state.options.times.firstTime); // 19 мая(старт) среда
      for (let weekIndex = 0; weekIndex < weeksCount.count; weekIndex++) { // для каждой недели
        let weekWidth = 0;
        let weekOffset = Number.MAX_SAFE_INTEGER;
        let finalDate = dayjs_min_default()(currentDate) // начальный день каждой недели
          .add(1, 'week')
          .startOf('week');
        ...
        for (let formatName of formatNames) {
          if (this.root.state.options.calendar.week.maxWidths[formatName] + 2 <= weekWidth) {  // long, medium, short
        ...
        currentDate = currentDate.add(1, 'week').startOf('week'); // каждый понедельник
        if (currentDate.valueOf() > this.root.state.options.times.lastTime) {
          currentDate = dayjs_min_default()(this.root.state.options.times.lastTime); // окончание графика
        }
      }
...
const weekOfYear = __webpack_require__(5);
dayjs_min_default.a.extend(weekOfYear);
const quarterOfYear = __webpack_require__(6);
dayjs_min_default.a.extend(quarterOfYear);
const advancedFormat = __webpack_require__(7);
dayjs_min_default.a.extend(advancedFormat);
